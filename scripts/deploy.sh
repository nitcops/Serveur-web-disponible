...
docker exec web1 bash -c "echo '<h1>Serveur n°1 - Prod A</h1>' > /usr/share/nginx/html/index.html"
docker exec web2 bash -c "echo '<h1>Serveur n°2 - Prod B</h1>' > /usr/share/nginx/html/index.html"
